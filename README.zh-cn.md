...
只有 `sources` 与变更文件存在交集的条目才会被处理。更新某个条目绝不会清除其他条目的过期状态。写入是原子的，并由短生命周期锁保护，因此多个 agent 可以安全地共享记忆库。配置项：`.agent-memoize/config.json` 中的 `staleness`（或 `MEMOIZE_STALENESS` 环境变量）： `strict` | `selective`，默认 `selective`。`ignoreComments: true` 还会在计算规范化哈希时，按语言剥离注释（行注释与块注释，并保留其周围的代码）。
